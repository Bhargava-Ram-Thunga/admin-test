...
const AnalyticsCard = ({ config, onUpdate, onRemove }: any) => {
        ...
        <Card className="flex flex-col h-[480px] group relative overflow-visible transition-all hover:shadow-xl animate-in fade-in zoom-in duration-300">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-6 p-4 bg-[#F5F7FA] rounded-2xl border border-[#393D7E]/5">
                <CustomSelect
                    label="Graph Type"
                    value={config.type}
                    onChange={(val) => onUpdate(config.id, { type: val })}
                ...
                <CustomSelect
                    ...
                    onChange={(val) => onUpdate(config.id, { xAxisMode: val })}
                    options={["time", "region"]}
                ...
                {config.xAxisMode === "time" ? (
                    <CustomSelect
                        ...
                        onChange={(val) => onUpdate(config.id, { timeGranularity: val })}
                ...
                <CustomSelect
                    label="Metric"
                    value={config.metric}
                    onChange={(val) => onUpdate(config.id, { metric: val })}
