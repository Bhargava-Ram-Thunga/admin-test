...
export const RegionsView = ({ user, students }: any) => {
    ...
    const availableLevels = useMemo(() => {
        return HIERARCHY_LEVELS.filter((_, index) => index > adminLevelIndex!);
    }, [adminLevelIndex]);
    ...
    const availableLocations = useMemo(() => {
        ...
        const findNode: any = (nodes: any[], id: string) => {
    ...
    const filteredStudents = useMemo(
        () =>
            !selectedLocationId
                ? []
                : students.filter((s: any) => locationDataIds.includes(s.regionId)),
    ...
    const filteredTeachers = useMemo(
        () =>
            !selectedLocationId
                ? []
                : MOCK_TRAINERS.filter((t: any) => locationDataIds.includes(t.regionId)),
        [locationDataIds]
    );
            ...
            <div className="bg-white p-6 rounded-3xl shadow-lg border border-[#393D7E]/5 flex flex-col md:flex-row items-center gap-6 justify-between">
                <div className="flex flex-col md:flex-row gap-4 w-full md:w-auto">
                    ...
                    <div className="flex flex-col gap-1 w-full min-w-[200px]">
                        <label
                            className={`text-[10px] font-bold uppercase tracking-wider pl-1 ${!selectedType ? "text-gray-300" : "text-[#393D7E]/50"
                                }`}
                        >
                            Location
                        </label>
                        <div className="relative group">
                            <MapPin
                                size={14}
                                className={`absolute left-3 top-1/2 -translate-y-1/2 ${!selectedType ? "text-gray-300" : "text-[#393D7E]/40"
                                    }`}
                            />
                            <select
                            ...
                            <ChevronDown
                                size={14}
                                className={`absolute right-3 top-1/2 -translate-y-1/2 pointer-events-none ${!selectedType ? "text-gray-300" : "text-[#393D7E]/40"
                                    }`}
                            />
                        </div>
                    </div>
