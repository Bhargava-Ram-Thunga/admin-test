...
export const getDescendantIds = (node: any): string[] => {
    let ids = [node.id];
    if (node.children) {
...
export const getDescendantsByType = (node: any, targetType: string): any[] => {
    let results: any[] = [];
...
export const getScopeIds = (user: any): string[] => {
...
export const checkPermission = (user: any, itemRegionId: string): boolean => {
...
export const getLevelIndex = (type: string): number => HIERARCHY_LEVELS.indexOf(type);

// Determines the user's hierarchy level index based on their assigned node
export const getAdminLevelIndex = (user: any): number | null => {
    if (user.regionId === "ALL") return -1; // Super Admin (above State)

    const findNodeLevel = (nodes: any[], id: string): number | null => {
        for (const node of nodes) {
...
export const generateAnalyticsData = (config: any) => {
